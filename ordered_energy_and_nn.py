...
if __name__ == "__main__":
    ...
    vor_sides1=plot_voronoi_tessellation(vor_data,xp,yp,max_area=1000)
    
    #Calculate particle energies
    kspring = 50
    energy_data1=calc_energy(xp,yp,xd,yd,kspring,pin_mag,radius,pin_rad)

    energy_data, vor_sides= zip(*sorted(zip(energy_data1, vor_sides1)))

    sum_energy = 0
    
    for j in range(len(xp)):
        sum_energy += energy_data[j]

    avg = sum_energy/len(xp)
    
    normalized_energy = []
    for k in range(len(xp)):
        normalized_energy.append(energy_data[k] -  avg)

    print("Writing to File OrderedEnergies.txt")
    for i in range(len(normalized_energy)):
        with open(out_file1, "a") as myfile:
            myfile.write("%f" %vor_sides[i])
            myfile.write(" ")
            myfile.write("%f" %normalized_energy[i])
            myfile.write("\n")
    print("done")
